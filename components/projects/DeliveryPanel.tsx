...
import { ProjectDelivery, AiDeliveryResult } from '@/types/freelancer';
...
export function DeliveryPanel({ projectId, delivery: initialDelivery, onUpdate }: DeliveryPanelProps) {
  ...
  const [result, setResult] = useState<AiDeliveryResult | null>(null);
  ...
  const summary = result?.delivery_summary ?? delivery?.delivery_summary;
  const clientMessage = result?.client_message ?? delivery?.client_message;
  const assets = result?.assets ?? delivery?.assets_json ?? [];
  const nextSteps = result?.next_steps ?? delivery?.next_steps_json ?? [];
    ...
    <div className="space-y-5">
      ...
      {clientMessage && (
        <div className="bg-bg-card border border-accent-blue/20 rounded-xl p-5">
          ...
          <p className="text-sm text-slate-200 whitespace-pre-wrap leading-relaxed">{clientMessage}</p>
          <button
            onClick={() => {
              navigator.clipboard.writeText(clientMessage).catch(() => {});
            }}
