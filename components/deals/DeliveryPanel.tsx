...
const deliveryTypes = ['figma_link', 'zip_link', 'live_url', 'doc', 'other'];
...
export function DeliveryPanel({ dealId, milestones, deliveries, onUpdate }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!milestoneId || !assetUrl) { setError('Milestone ve URL gerekli'); return; }
    setLoading(true); setError('');
    try {
      const res = await fetch(`/api/deals/${dealId}/deliveries`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ milestoneId, deliveryType, assetUrl, note }),
      });
  ...
  return (
    <div className="space-y-6">
      <Card className="p-5">
        <h3 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-4">Yeni Teslimat</h3>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs text-slate-400 mb-1 block">Milestone *</label>
              <select value={milestoneId} onChange={(e) => setMilestoneId(e.target.value)} className="w-full px-3 py-2 bg-bg-deep border border-white/10 rounded-lg text-sm text-white focus:outline-none focus:border-accent-blue/50" required>
            ...
            <div>
              ...
              <select value={deliveryType} onChange={(e) => setDeliveryType(e.target.value)} className="w-full px-3 py-2 bg-bg-deep border border-white/10 rounded-lg text-sm text-white focus:outline-none focus:border-accent-blue/50">
          ...
          <div>
            ...
            <Input value={assetUrl} onChange={(e) => setAssetUrl(e.target.value)} placeholder="https://..." required />
          </div>
          <div>
            <label className="text-xs text-slate-400 mb-1 block">Not</label>
            <Input value={note} onChange={(e) => setNote(e.target.value)} placeholder="Teslim notu..." />
          </div>
      ...
      {deliveries.length > 0 && (
        <div className="space-y-3">
          <h3 className="text-sm font-semibold text-slate-400 uppercase tracking-wider">Teslim Geçmişi</h3>
          {deliveries.map((d) => {
            ...
            return (
              <Card key={d.id} className="p-4">
                <div className="flex items-start gap-3">
                  ...
                  <span className="text-xs text-slate-600 flex-shrink-0">{new Date(d.created_at).toLocaleDateString('tr-TR')}</span>
                </div>
              </Card>
            );
          })}
