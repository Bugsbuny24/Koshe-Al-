...
export function DealCard({ deal }: Props) {
  const activeMilestone = deal.deal_milestones?.find((m) => m.status === 'in_progress') || deal.deal_milestones?.[0];
      ...
      <Card hover className="p-5 space-y-3">
        ...
        {escrow && (
          <div className="text-xs text-slate-500">
            Escrow: <span className="text-slate-300">{escrow.status}</span>
            {escrow.funded_amount > 0 && <span className="ml-1">· {deal.currency} {escrow.funded_amount.toLocaleString()} funded</span>}
          </div>
        )}
        <div className="text-xs text-slate-600">
          {new Date(deal.updated_at).toLocaleDateString('tr-TR', { day: '2-digit', month: 'short', year: 'numeric' })}
        </div>
      </Card>
    </Link>
  );
