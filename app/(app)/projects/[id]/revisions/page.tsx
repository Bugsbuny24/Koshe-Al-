...
import { Project, ProjectRevision } from '@/types/freelancer';
...
export default function RevisionsPage() {
  ...
  const load = () => {
    ...
    Promise.all([
      ...
      fetch(`/api/projects/${id}/revisions`).then((r) => r.json()),
    ]).then(([pd, rd]) => {
      if (pd.project) setProject(pd.project);
      if (rd.revisions) setRevisions(rd.revisions);
  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      <RevisionsPanel projectId={id} revisions={revisions} onUpdate={load} />
    </div>
  );
}
