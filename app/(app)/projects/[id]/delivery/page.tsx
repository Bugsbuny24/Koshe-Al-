...
import { Project, ProjectDelivery } from '@/types/freelancer';
...
export default function DeliveryPage() {
  ...
  const load = () => {
    ...
    Promise.all([
      ...
      fetch(`/api/projects/${id}/delivery`).then((r) => r.json()),
    ]).then(([pd, dd]) => {
      if (pd.project) setProject(pd.project);
      if (dd.delivery) setDelivery(dd.delivery);
  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      <DeliveryPanel projectId={id} delivery={delivery} onUpdate={load} />
    </div>
  );
}
